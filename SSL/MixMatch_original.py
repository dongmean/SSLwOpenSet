...
def train(labeled_trainloader, unlabeled_trainloader, ood_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda):
    ...
    for batch_idx in range(args.train_iteration):
        ...
        # TAUFE
        if args.reg_weight > 0:
            outputs_ood, features_ood = ALOOD_OURS(inputs_ood[0].cuda())

            target_random = torch.LongTensor(inputs_ood.shape[0]).random_(0, args.n_class).cuda()
            loss = self.loss_function(logits[:in_data.shape[0]], target) + self.args.lambda_o * F.cross_entropy(
                logits[in_data.shape[0]:], target_random)
