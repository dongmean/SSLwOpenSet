...
def load_datasets(DATASET_PATH, HAS_DATASET, index):
    ...
    if index == True:
        if not HAS_DATASET:
            ...
            # svhn
            file_path = '../../data/svhn/'
            svhn_train = MySVHN(file_path, download=False, transform=TransformTwice(train_transform))
            svhn_unlabeled = MySVHN(file_path, download=False, transform=TransformTwice(test_transform))

            # LSUN
            file_path = '../../data/lsun/lsun_x_sample_50000_np.pickle'
            #lsun_train = MyLSUN(file_path, transform=TransformTwice(tiny_transform))
            #lsun_unlabeled = MyLSUN(file_path, transform=TransformTwice(tiny_test_transform))
