...
from dataset.cifar_fm import DATASET_GETTERS, get_ood
...
def set_model_config(args):
    ...
    if args.dataset == 'cifar10':
        args.ood_data = ['svhn'] #["svhn", 'cifar100', 'lsun', 'imagenet']
...
def set_dataset(args):
    labeled_dataset, unlabeled_dataset, test_dataset = \
        DATASET_GETTERS[args.dataset](args, args.root)

    train_ood = get_ood(args)
    ...
    if args.local_rank not in [-1, 0]:
        ...
        multi_unlabeled_datasets = torch.utils.data.ConcatDataset(datalist)

    return labeled_trainloader, multi_unlabeled_datasets, \
           test_loader
